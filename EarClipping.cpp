...
bool ***ptr;
bool ***vis;
vector<vector<point> > cc;// stores the functional component
vector<vector<point> > grp;// stores the digital planar functional component
vector<point> neighbors(int x,int y,int z);
int value(vector<point> &p);
void visualisation(ofstream &outdata);
void connected();
vector<point> bfs(int f,point p);
void triangle();
void partiton(vector<point>v);
bool test(planeinfo p,point t);
void red(ofstream &outdata);
vector<point>  help(vector<pair<int,int> >&v1,vector<pair<int,int> >&v2,int flag);
void red(ofstream &outdata)
{
...
bool test(planeinfo p,point t)
{
   int z=p.A*t.x+p.B*t.y+p.C*t.z+p.D;
   int l=max(p.A,max(p.B,p.C));
   if(2*z>=-1*l && 2*z<=l)
      return true;
   else
      return false;
...
void partition(vector<point>&v)
{
   /*
   1. Take 3 points and form the equation of real plane 
   2. Take all points in a group that satisfy the digital plane equation.
   3. Form convex Hull of these points
   4. Apply ear clipping algorithm of the points obtained after applying convex Hull.
   */
   /*
      Take 3 points and form the quation of plane with it.
      Add all points which satisfy this equation.
      Remove these points and then again apply the same preocedure
   */
   int i;
   set<pair<int,pair<int,int> > > xyz;
   for(i=0;i<v.size();i++)
      xyz.insert({v[i].x,{v[i].y,v[i].z}});
   while(xyz.size()>2)
   {
       point p[3];
       int i=0;
       vector<point> g;
       for(auto it=xyz.begin();it!=xyz.end() && i<3;it++)
       {
         pair<int,pair<int,int> > l=*it;
         p[i].x=l.first;
         p[i].y=l.second.first;
         p[i].z=l.second.second;
         i++;
       }
...
void triangle()
{
   // cc[i] has functional components 
   for(auto &v:cc)
   {
      if(v.size()>=3)
      partition(v);
      ...
      while(sz--)
      {
         ...
         for(int i=-1;i<=1;i++)
         {
            for(int j=-1;j<=1;j++)
            {
               for(int k=-1;k<=1;k++)
               {
                  if(i+t.x>=0 && i+t.x<=x_m-x_l && j+t.y>=0 && j+t.y<=y_m-y_l
                     && k+t.z>=0 && k+t.z<=z_m-z_l && ptr[t.x+i][t.y+j][t.z+k]
                     && !vis[i+t.x][j+t.y][k+t.z] && fp[i+t.x][j+t.y][k+t.z]==f)
                  {
                     vis[i+t.x][j+t.y][k+t.z]=true;
                     point w;
                     w.x=i+t.x;
                     w.y=j+t.y;
                     w.z=k+t.z;
...
void connected()
{
   ...
   for(i=0;i<=x_m-x_l;i++)
   {
      for(j=0;j<=y_m-y_l;j++)
      {
         for(k=0;k<=z_m-z_l;k++)
         {
            if(!vis[i][j][k] && ptr[i][j][k])
               {
                  point p;
                  p.x=i;
                  p.y=j;
                  p.z=k;
                  vector<point>v=bfs(fp[i][j][k],p);
                  cc.push_back(v);
               }
         }
      }
...
{
   // ofstream outdata;
   // outdata.open("Digital_planar.obj"); // opens the file
...
{
   /*
    Take a voxel . Get all its neighbors + voxel itself. Project the voxel into
    xy, yz , zx plane. The max(xy,yz,zx) will be the functional plane for the voxel .
    Follow the similar procedure for all voxels to get the functional plane 
    corresponding to each voxel. 
   */

   int i,j,k;
   fp=new int**[x_m-x_l+1];
   for(i=0;i<=x_m-x_l;i++)
   {
      fp[i]=new int*[y_m-y_l+1];
      for(j=0;j<=y_m-y_l;j++)
      {
         fp[i][j]=new int[z_m-z_l+1];
      }
   }
   for(i=0;i<=x_m-x_l;i++)
   {
...
{
      ifstream in;  
      in.open("botijo.obj"); // taking input of vertices and faces from the orignal.obj file and scaling it
      if(!in) { 
         cerr << "Error: file could not be opened" << endl;
         exit(1);
      }
   ofstream out;
   out.open("mbotijo.obj"); // Storing the scaled version of orignal .obj file in this file
...
int main()
{
   ...
   connected();
   triangle();
   // Sort the points and digitalise the two extreme points and visualise them in red
   for(int i=0;i<grp.size();i++)
   {
      sort(grp[i].begin(),grp[i].end(),f);
   }
   // Similar to visualisation
   ofstream outdata;
   outdata.open("Digital_planar.obj"); // opens the file
   if( !outdata ) { // file couldn't be opened
      cerr << "Error: file could not be opened" << endl;
