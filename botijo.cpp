...
{
   ofstream outdata;
   outdata.open("BotijoFunctionalComponent.obj"); // opens the file
...
{
   ofstream outdata;
   outdata.open("Botijo.obj"); // opens the file
   if( !outdata ) { // file couldn't be opened
      cerr << "Error: file could not be opened" << endl;
      exit(1);
   }
...
void scaling()
{
   ...
   while ( !in.eof() )
   {
         ...
         if(q=='v')
         {
            ...
            in>>x>>y>>z;
            int x1=int(round(x*10));// Scaling factor of 5 used
            int y1=int(round(y*10));// round function used as we need to operate on integer coordinates only
            int z1=int(round(z*10));
