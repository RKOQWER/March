/*
	Storing the voxels correspondig to  a single triangle beforehand
*/ 
...
vector<point> voxelization(vector<pair<int,int> > &pixels,int A,int B,
...
{
   ofstream outdata;
   outdata.open("BunnyFunctionalComponent.obj"); // opens the file
...
{
   ofstream outdata;
   outdata.open("bunny_planar.obj"); // opens the file
   if( !outdata ) { // file couldn't be opened
      cerr << "Error: file could not be opened" << endl;
      exit(1);
   }
...
void input(point a,point b,point c,ifstream &in)
{
   ...
   else
   {
      ...
      int flag=-1; 
       ...
       vector<point> v=voxelization(pixels,p.A,p.B,p.C,p.D,flag);
       grp.push_back(v);
       pixels.clear();
    }
...
{
      ifstream in;  
      in.open("bunny.obj"); // taking input of vertices and faces from the orignal.obj file and scaling it
      if(!in) { 
         cerr << "Error: file could not be opened" << endl;
         exit(1);
      }
   ofstream out;
   out.open("mbunny.obj"); // Storing the scaled version of orignal .obj file in this file
   ...
   while ( !in.eof() )
   {
         ...
         if(q=='v')
         {
            ...
            in>>x>>y>>z;
            int x1=int(round(x*350));// Scaling factor of 5 used
            int y1=int(round(y*350));// round function used as we need to operate on integer coordinates only
            int z1=int(round(z*350));
